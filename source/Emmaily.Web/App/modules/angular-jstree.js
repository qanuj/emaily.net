(function (angular) {
    ...
    function jsTreeDirective() {
        return {
            ...
            scope: {
                treeData: '=ngModel',
                treeChecked: '=vzChecked',
                treeDisabled: '=ngDisabled',
            ...
            link: function link(scope, elm, attrs, controller) {
                ...
                function setCheckedItems(node, selected) {
                    scope.treeChecked = selected.selected;
                    scope.$apply();
                }

                scope.init = function () {
                    scope.tree.jstree(config);
                    if (scope.treeChecked && scope.treeChecked.length>-1) {
                        scope.tree.on('check_node.jstree', setCheckedItems);
                        scope.tree.on('open_all.jstree', function () {
                            if (scope.treeChecked && scope.treeChecked.length > -1) {
                                scope.tree.jstree(true).check_node(scope.treeChecked);
                            }
                        });
                        scope.tree.on('uncheck_node.jstree', setCheckedItems);
                    }
                    scope.tree.on('loaded.jstree', function () {
                        scope.tree.jstree(true).open_all();
                    });
